 ...
 */
export function facetAwareSegment(text, length, facets) {
    ...
    for (let i = 0; i < segments.length; i++) {
        ...
        if (currentSubstring.length >= length) {
            ...
            const facetBeingBroken = facets.find((facet) => facet.index.byteStart < currentGraphemeUtf8Index
                && currentGraphemeUtf8Index < facet.index.byteEnd);
            // Backtrack until we're no longer within the byte range of the facet
            while (currentGraphemeUtf8Index >= (facetBeingBroken?.index.byteStart ?? Infinity)
            ...
            const substringFacets = facets.reduce((acc, facet) => {
                const { byteStart, byteEnd } = facet.index;
                if (byteStart >= utf16IndexToUtf8Index(text, currentSubstring[0].index)
