...
import { PostReference } from "./PostReference.js";
...
export class Post extends PostReference {
    ...
    constructor(
    // dprint-ignore
    { text, uri, cid, author, facets, replyRef, langs, embed, labels, tags, likeUri, repostUri, likeCount, replyCount, repostCount, quoteCount, threadgate, embeddingDisabled, createdAt = new Date(), indexedAt, parent, root, children, }, bot) {
        ...
        if (embeddingDisabled !== undefined)
    ...
    async fetchThreadView() {
        ...
        if (response.data.thread.$type !== "app.bsky.feed.defs#threadViewPost") {
            throw new Error(`Could not fetch post ${this.uri}. ` + response.data.thread.$type
     ...
     */
    async fetch(options = {}) {
        return Object.assign(this, await this.bot.getPost(this.uri, { skipCache: true, ...options }));
    }
    /**
     * Fetch the root post of the thread.
     * @param options Optional configuration.
     */
    async fetchRoot({ force = false } = {}) {
        if (this.root && !force)
            return this.root;
        if (!this.replyRef?.root?.uri)
            return null;
     ...
     * @param options Optional configuration.
     */
    async fetchParent({ parentHeight = 1, force = false } = {}) {
        if (this.parent && !force)
            return this.parent;
        if (!this.replyRef?.parent?.uri)
            return null;
     ...
     * @param options Optional configuration.
     */
    async fetchChildren({ depth = 1, force = false } = {}) {
    ...
    setCounts(view) {
        if (view.likeCount != undefined)
            this.likeCount = view.likeCount;
        if (view.repostCount != undefined)
            this.repostCount = view.repostCount;
        if (view.replyCount != undefined)
            this.replyCount = view.replyCount;
        if (view.quoteCount != undefined)
            this.quoteCount = view.quoteCount;
    }
    /**
    ...
    static fromView(view, bot) {
        if (!is("app.bsky.feed.post", view.record))
     ...
     */
    static fromThreadView(view, bot) {
        ...
        const parent = view.parent?.$type === "app.bsky.feed.defs#threadViewPost"
            ...
            : undefined;
        const children = view.replies?.map((reply) => reply.$type === "app.bsky.feed.defs#threadViewPost"
            ? Post.fromThreadView(reply, bot)
            : undefined)?.filter((reply) => reply !== undefined);
