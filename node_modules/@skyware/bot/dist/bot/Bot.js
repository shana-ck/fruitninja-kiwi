...
export class Bot extends EventEmitter {
     ...
     */
    constructor({ service = "https://bsky.social", langs = ["en"], emitEvents = true, emitChatEvents = false, rateLimitOptions, cacheOptions, eventEmitterOptions = { strategy: EventStrategy.Polling }, chatEmitterOptions, } = {}) {
        super();
        this.handler = new CredentialManager({ service });
        this.agent = new RateLimitedAgent({ handler: this.handler }, new RateLimitThreshold(3000, rateLimitOptions?.rateLimitInterval ?? 300));
        this.langs = langs;
        this.cache = {
    ...
    async getLabelers(dids, options = {}) {
        const response = await this.agent.get("app.bsky.labeler.getServices", {
            params: { dids: dids, detailed: true },
        }).catch((e) => {
    ...
    async post(payload, options = {}) {
        ...
        if (payload.facets?.length) {
            facets = payload.facets.map((facet) => facet instanceof Facet ? facet.toRecord() : facet);
        }
        ...
        const uploadMedia = async (media, mimeTypePrefix) => {
            let blob;
            ...
            if (!blob.type)
                throw new Error("Must provide a content type for media data.");
        ...
        if (payload.quoted) {
            ...
            if (payload.video?.data instanceof Blob && !payload.video.data.type.startsWith("video/")) {
    ...
    async leaveConversation(id) {
        ...
        await this.chatProxy.call("chat.bsky.convo.leaveConvo", { data: { convoId: id } }).catch((e) => {
    ...
    async negateLabels({ reference, labels, blobCids = [], comment }) {
        return this.emitLabelEvent(reference, {
            ...
            throw new Error(`Failed to negate label on record ${"did" in reference ? reference.did : reference.uri}.`, { cause: e });
        });
    }
    async emitLabelEvent(reference, event, subjectBlobCids) {
        ...
        return this.agent.withProxy("atproto_labeler", this.profile.did).call("tools.ozone.moderation.emitEvent", {
            data: {
                event: {
    ...
    async updateHandle(handle) {
        if (!this.hasSession)
            throw new Error(NO_SESSION_ERROR);
        await this.agent.call("com.atproto.identity.updateHandle", { data: { handle } }).catch((e) => {
     ...
     */
    async updatePreferences(callback) {
        if (!this.hasSession)
            throw new Error(NO_SESSION_ERROR);
