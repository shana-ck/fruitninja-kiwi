...
export class BotEventEmitter extends EventEmitter {
    ...
    constructor(options, bot) {
        ...
        if (this.strategy === EventStrategy.Firehose) {
            ...
            import("@skyware/jetstream").then(({ Jetstream }) => {
                ...
                throw new Error("Failed to import Jetstream event emitter. Make sure you have the @skyware/jetstream package installed.");
            });
        }
            ...
            (async () => {
                for (const op of message.ops) {
                    ...
                    if (is("app.bsky.feed.post", op.record)) {
                        ...
                        const isQuoteWithMedia = is("app.bsky.embed.recordWithMedia", op.record.embed)
                        ...
                        // Mention
                        if (op.record.facets?.some((facet) => facet.features.some((feature) => is("app.bsky.richtext.facet#mention", feature)
                            && feature.did === this.bot.profile.did)) && this.listenerCount("mention") >= 1) {
                            const post = await this.bot.getPost(uri);
    ...
    startJetstream() {
        ...
        this.jetstream?.onCreate("app.bsky.feed.post", async ({ commit: { record, rkey }, did }) => {
            const uri = `at://${did}/app.bsky.feed.post/${rkey}`;
            if (record.reply?.parent?.uri?.includes(`at://${this.bot.profile.did}`)) {
                this.emit("reply", await this.bot.getPost(uri));
            }
            else if (is("app.bsky.embed.record", record.embed)
                && record.embed.record.uri.includes(`at://${this.bot.profile.did}`)) {
                this.emit("quote", await this.bot.getPost(uri));
            }
            else if (is("app.bsky.embed.recordWithMedia", record.embed)
                && record.embed.record.record.uri.includes(`at://${this.bot.profile.did}`)) {
                this.emit("quote", await this.bot.getPost(uri));
            }
            else if (record.facets?.some((facet) => facet.features.some((feature) => is("app.bsky.richtext.facet#mention", feature)
                && feature.did === this.bot.profile.did))) {
                this.emit("mention", await this.bot.getPost(uri));
            }
        });
        this.jetstream?.onCreate("app.bsky.feed.repost", async ({ commit: { record, rkey }, did }) => {
        ...
        this.jetstream?.onCreate("app.bsky.feed.like", async ({ commit: { record, rkey }, did }) => {
        ...
        this.jetstream?.onCreate("app.bsky.graph.follow", async ({ commit: { record, rkey }, did }) => {
    ...
    async poll() {
        ...
        for (const notification of newNotifications) {
            switch (notification.reason) {
                ...
                case "repost": {
                    if (!is("app.bsky.feed.repost", notification.record)
                ...
                case "like": {
                    if (!is("app.bsky.feed.like", notification.record)
                        || !notification.reasonSubject) {
                        emitInvalidRecordError(notification);
                        break;
                    }
